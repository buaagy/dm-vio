...
#define SCALE_B_INVERSE (1.0f / SCALE_B)
	...
	struct FrameHessian
	{
		...
		Eigen::Vector3f *dI;			   // 指向 dIp[0],即原始图像的信息,用于fine tracking. Used for direction select (not for gradient histograms etc.)
		Eigen::Vector3f *dIp[PYR_LEVELS];  // 存储每一层图像的辐照度,x和y方向梯度,用于coarse tracking,NAN in [0] only.
		float *absSquaredGrad[PYR_LEVELS]; // 存储xy方向梯度的平方和,only used for pixel select (histograms etc.). no NAN.
