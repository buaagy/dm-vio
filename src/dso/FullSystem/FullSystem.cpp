...
namespace dso
{
	...
	*/
	...
	/*
	DSO和DM-VIO的入口就是FullSystem::addActiveFrame。
	对于输入图像会做预处理,如果有光度标定,像素值不是灰度值,而是处理后的辐照度,取值范围是[0,255],float型。
	预处理是调用FrameHessian::makeImages函数,为当前帧的图像建立图像金字塔,并计算每一层图像的梯度。
	另外会创建FrameSheel,是FrameHessian的成员变量,用来保存位姿信息。
	fh指针指向当前帧的FrameHessiam,在处理完成当前帧之后,会删除FramHessian,而保存信息到变量allFrmaeHistory中,作为整条轨迹的输出。
	另外也会传入上一帧到当前帧之间的所有IMU数据,以及ground-truth数据。
	*/
	void FullSystem::addActiveFrame(ImageAndExposure *image, int id, dmvio::IMUData *imuData, dmvio::GTData *gtData)
	{
		...
		// 创建FrameHessian和FrameShell的对象,将shell赋给fh,并添加到allFrameHistory中
