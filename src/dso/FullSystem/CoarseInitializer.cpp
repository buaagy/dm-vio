...
namespace dso
{
	...
	其它层则选出goodpoints,作为后续第二帧匹配生成pointHessians和immaturePoints的候选点,这些点存储在CoarseInitializer::points中。
	每一层点之间都有联系,在CoarseInitializer::makeNN中计算每个点最邻近的10个点neighbours,在上一层的最邻近点parent。
	pointHessians是成熟点,具有逆深度信息的点,能够在其他帧中追踪到的点。
	immaturePoints是未成熟点,需要使用非关键帧的图像对它的逆深度进行优化,再使用关键帧将它转换成pointHessians,并且加入到窗口优化。
	*/
